...
def test_if_none_match_returns_304(invoke: Invoke) -> None:
...
def test_304_validates_against_spec(
...
def test_stale_etag_returns_200(invoke: Invoke) -> None:
    result = invoke(
        FORWARD,
...
def test_siblings_carries_etag_bare_reverse_does_not(invoke: Invoke) -> None:
    siblings = invoke(
        REVERSE,
