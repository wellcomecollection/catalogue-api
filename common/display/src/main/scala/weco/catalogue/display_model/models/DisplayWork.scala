...
import weco.catalogue.internal_model.work.{Relation, Work, WorkState, WorkType}
...
object DisplayWork {
  ...
  def apply(
    ...
    DisplayWork(
      ...
      partOf =
        if (includes.partOf) {
          Some(constructPartOfHierarchy(work.state.relations.ancestors))
        } else None,
      parts =
        if (includes.parts)
          Some(
   ...
   *
   */
  private def constructPartOfHierarchy(flatAncestors: List[Relation]): List[DisplayRelation] = {
    flatAncestors.foldLeft(List.empty[DisplayRelation]) {
      case (List(), nearer) =>
        List(DisplayRelation(nearer))
      case (head::tail, nearer:Relation) => (head.id, nearer.id) match {
