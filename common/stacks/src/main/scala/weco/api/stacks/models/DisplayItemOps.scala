...
trait DisplayItemOps {
  implicit class DisplayItemOps(item: DisplayItem) {
    ...
    /** Get the physical location for an item.
      *
      * In practice we know an item will only have at most one physical
      * location.
      */
    ...
    /** Get the physical access condition for an item.
      *
      * In practice we know an item will only ever have a single access
      * condition.
      */
    ...
    /** There are two cases we care about where the data in the catalogue API
      * might be stale:
      *
      * 1) An item was on request for a user, but has been returned to the
      * stores. Another user could now request the item, but the catalogue API
      * will tell you it's temporarily unavailable.
      *
      ...
      *
      * In all other cases, we can use the access conditions in the catalogue
      * API. There may be a small delay in updating an item's information, but
      * this is relatively tolerable, and allows us to simplify the logic in
      * this API for determining item status.
      *
