...
class ItemLookupTest extends AnyFunSpec with Matchers with EitherValues with IndexFixtures with ItemsGenerators with WorkGenerators {
  ...
  it("finds a work with the same item ID") {
    ...
    withLocalWorksIndex { index =>
      ...
      val future1 = lookup.byId(item1.id.canonicalId)(index)

      whenReady(future1) {
        _ shouldBe Right(Some(item1))
      }

      val future2 = lookup.byId(item2.id.canonicalId)(index)

      whenReady(future2) {
        _ shouldBe Right(Some(item2))
      }

      val future3 = lookup.byId(item3.id.canonicalId)(index)
  ...
  it("returns None if there is no visible item") {
    ...
    withLocalWorksIndex { index =>
      ...
      val future1 = lookup.byId(item.id.canonicalId)(index)
      ...
      val future2 = lookup.byId(item.id.canonicalId)(index)
