...
class WorksService(val elasticsearchService: ElasticsearchService)(
...
) extends SearchService[Work[Indexed], Work.Visible[Indexed], WorkAggregations, WorkSearchOptions] with ElasticAggregations {
  ...
  def countWorkTypes(index: Index): Future[Either[ElasticsearchError, Map[String, Int]]] = {
    ...
    searchResponse.map {
      case Right(resp) => Right(
        resp.aggregations.decodeAgg[String]("work_type")
          .get
          .buckets
          .map {
