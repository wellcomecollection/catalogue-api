...
trait Encoders {
  ...
  implicit val aggregationEncoder: Encoder[AbstractAggregation] = agg =>
    parser.parse(JacksonBuilder.writeAsString(AggregationBuilderFn(agg).value)).getOrElse(Json.obj())
  ...
  implicit val queryEncoder: Encoder[Query] = query =>
    parser.parse(JacksonBuilder.writeAsString(QueryBuilderFn(query).value)).getOrElse(Json.obj())
