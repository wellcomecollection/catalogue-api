...
import weco.api.search.config.builders.{MultiClusterElasticClientBuilder, PipelineElasticClientBuilder}
...
object MultiClusterMain extends WellcomeTypesafeApp {
  ...
  runWithConfig { config: Config =>
    ...
    val (defaultElasticClient, defaultElasticConfig) = apiConfig.environment match {
    ...
    val router: ApiRouter = if (additionalClients.nonEmpty) {
      info(s"Using multi-cluster router with ${additionalClients.size} additional cluster(s)")
      ...
      info("No additional clusters configured, using standard single-cluster router")
      new SearchApi(
