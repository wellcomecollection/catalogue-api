...
object MultiClusterConfigParser extends Logging {
  ...
  def parseMultiClusterConfig(
    ...
    val additionalClusters: Map[String, ClusterConfig] = clusterNames.map {
      clusterName =>
        val clusterConfig = parseClusterConfig(
          clusterName,
          multiClusterConfig.getConfig(clusterName))
  ...
  private def parseClusterConfig(name: String,
    ...
    val customApiKeySecretPath =
      getStringOption(config, "customApiKeySecretPath")

    info(
      s"Parsed cluster config '$name': " +
