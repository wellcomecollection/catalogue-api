...
class ItemUpdateService(
  ...
  private def buildItemsMap(items: Seq[Item[IdState.Identified]]): Map[SourceIdentifier, Item[IdState.Identified]] =
    items.map {
      case item@Item(IdState.Identified(_, srcId, _), _, _, _) => srcId -> item
    } toMap

  private def preserveOrder(itemsWithIndex: Seq[(Item[IdState.Identified], Int)], f: Seq[Item[IdState.Identified]] => Future[Seq[Item[IdState.Identified]]]) = {
  ...
  def updateItems(
    ...
    Future.sequence {
      work.data.items.zipWithIndex.groupBy {
        case (Item(IdState.Identified(_, srcId, _), _, _, _), _) => Some(srcId.identifierType)
        ...
        case (Some(identifierType), itemsWithIndex: Seq[(Item[IdState.Identified], Int) @unchecked]) => itemUpdatesMap
            .get(identifierType)
