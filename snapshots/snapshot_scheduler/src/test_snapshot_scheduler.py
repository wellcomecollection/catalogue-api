...
def test_writes_messages_to_sqs(
    ...
    works_snapshot_job = next(
        m for m in messages if m["index"]["name"] == test_indices["works"]
    )
    images_snapshot_job = next(
        m for m in messages if m["index"]["name"] == test_indices["images"]
    )

    assert (
        works_snapshot_job["s3Location"]["key"]
        == f"{public_object_key_prefix}/works.json.gz"
    )
    assert (
        images_snapshot_job["s3Location"]["key"]
        == f"{public_object_key_prefix}/images.json.gz"
    )
