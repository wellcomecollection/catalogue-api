...
class S3Uploader(partSize: Int = (5 * FileUtils.ONE_MB).toInt)(implicit s3Client: AmazonS3) extends Logging {
  ...
  def upload(location: S3ObjectLocation, bytes: Iterator[Byte]): Try[CompleteMultipartUploadResult] = Try {
    val initResponse = s3Client.initiateMultipartUpload(
      new InitiateMultipartUploadRequest(location.bucket, location.key)
    )

    debug(s"Got init response for MultiPart Upload: upload ID ${initResponse.getUploadId}")
