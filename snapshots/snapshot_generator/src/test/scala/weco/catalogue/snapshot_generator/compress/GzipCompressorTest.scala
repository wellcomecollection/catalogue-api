...
class GzipCompressorTest extends AnyFunSpec with Matchers with GzipUtils with RandomGenerators {
  it("creates a gzip-compressed file") {
    val strings = (1 to 1000).map { _ => randomAlphanumeric(length = 1000) }
    ...
    GzipCompressor(strings.toIterator)
      .grouped(1024)
      .foreach { bytes => outStream.write(bytes.toArray) }
